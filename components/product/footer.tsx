...
const ALL_PLATFORMS = ["instagram", "youtube", "facebook", "tiktok", "twitter"] as const;
...
export default function Footer({ data }: { data: ProductData }) {
  ...
  return (
    <footer ref={sectionRef} className="relative pt-24 pb-12 overflow-hidden" style={{ background: "#080808" }}>
      ...
      {/* Social icons — now shows all platforms with icons */}
      <div className="flex justify-center gap-3 mb-16 flex-wrap px-6">
        {ALL_PLATFORMS.map((platform) => (
          <a key={platform} href={SOCIAL_URLS[platform] ?? "#"} target="_blank" rel="noopener noreferrer"
            aria-label={platform}
            className="w-11 h-11 rounded-full flex items-center justify-center transition-all duration-300 hover:bg-white/10 cursor-hover"
      ...
      <div className="max-w-[1000px] mx-auto px-6 lg:px-10 mb-16">
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-12">
          ...
          <div className="flex flex-col gap-3">
            ...
            {(["Contact", "Privacy Policy", "Terms of Service"] as const).map((l) => (
